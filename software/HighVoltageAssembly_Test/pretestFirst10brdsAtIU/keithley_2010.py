...
class Keithley_2010(object):
    ...
    def configure_voltage(self,acdc='DC'):
        ...
        if (acdc not in valid_set):
            ...
            print "here now!"
            #self.link.cmd("CONFIGURE:VOLTAGE:"+acdc)    #GV test hack
            self.link.cmd("VOLTAGE:"+acdc+":RANGE 9")    #GV test hack  # only allowed one setting per line, apparently!
            self.link.cmd("VOLTAGE:"+acdc+":NPLC 1")    #GV test hack
            self.link.cmd("FUNCTION 'VOLTAGE:DC'")
            self.link.cmd("INITIATE:CONTINUOUS OFF")
