...
unsigned int dacSleepUsecs = 10000;

int main ( int argc, char *argv[] ){
	if ( argc != 6 ) /* argc should be 2 for correct execution */
	{
        	/* We print argv[0] assuming it is the program name */
		printf( "usage: %s controlMode controlBoardType controlChan controlChanNum controlNumEvents", argv[0] );
		return 1;
	}
	int controlMode = atoi(argv[1]);
	int controlBoardType = atoi(argv[2]);
	int controlChan = atoi(argv[3]);
	int controlChanNum = atoi(argv[4]);
	int controlNumEvents = atoi(argv[5]);
	std::cout << "NEWRUN";
	std::cout << "\tmode " << controlMode;
	std::cout << "\tboard " << controlBoardType;
	std::cout << "\tchan " << controlChan;
	std::cout << "\tnum chan " << controlChanNum;
	std::cout << "\tnum events " << controlNumEvents;
	std::cout << std::endl;
	...
	for(int ch = controlChan ; ch < controlChan + controlChanNum ; ch++){
	//for(int ch = 1 ; ch < 3 ; ch++){
		...
		if( controlMode == 0 ){ //pedestal run
			...
			if( controlBoardType == 0 ) //electric board
				...
				sendControlWord(dev_handle, 0xAB, 0xC4, 0x30, dacValue[ch]); //Write/Update DAC A to ~1.9 V
			}
		}
		usleep(dacSleepUsecs);
